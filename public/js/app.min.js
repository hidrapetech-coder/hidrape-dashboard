...
const demoData = {
sensor: {
umidade: 68, temperatura: 24, bateria: 95, status: 'Online', sinal: 'Bom', bombaStatus: 'Desligada'
 }, clima: {
temp: 24.5, desc: "Parcialmente nublado", umidade: 62, vento: 12.3, chuva_1h: 0, previsao: [
 {
...
date: "Depois", temp_min: 20, temp_max: 26, pop: 60, weather_desc: "Chuva leve" }
 ...
 {
recomendacao: "A umidade do solo apresenta níveis adequados para a cultura monitorada. Manter o monitoramento atual e evitar irrigação adicional nas próximas horas.", insights: [
 "Solo retendo água de forma eficiente.", "Não há previsão de estresse hídrico para os próximos 3 dias."
 ]
 }, {
recomendacao: "Atenção: A umidade está caindo de forma consistente. Recomendado preparar sistema de irrigação para acionamento preventivo na próxima janela.", insights: [
 "Queda de 3% na umidade nas últimas 12 horas.", "Vento forte pode acelerar evapotranspiração."
 ]
 }
], iaIndex: 0, historico: Array.from({
length: 24 }, (_, i) => ({
horario: new Date(Date.now() - (23 - i) * 3600000).toISOString(), umidade: 65 + Math.random() * 5, temperatura: 22 + Math.random() * 4, status_bomba: i % 12 === 0 ? 'Ligada' : 'Desligada'
 }))
};
...
return Promise.resolve(new Response(JSON.stringify({
id: 'demo123', nome: 'Visitante (Demo)', empresa: 'Fazenda Hidrape', area_hectares: 42, cultura: 'Milho', data_cadastro: new Date().toISOString() }), {
status: 200 }));
}
if (urlStr.includes('/api/sensores/umidade')) {
return Promise.resolve(new Response(JSON.stringify(demoData.sensor), {
status: 200 }));
}
if (urlStr.includes('/api/agro/clima')) {
return Promise.resolve(new Response(JSON.stringify(demoData.clima), {
status: 200 }));
}
if (urlStr.includes('/api/agro/insights-ia')) {
const iaMock = demoData.ia[demoData.iaIndex % demoData.ia.length];
return Promise.resolve(new Response(JSON.stringify(iaMock), {
status: 200 }));
}
...
return Promise.resolve(new Response(JSON.stringify({
historico: demoData.historico.slice().reverse(), total: 24, page: 1, pages: 1 }), {
status: 200 }));
}
if (urlStr.includes('/api/auth/config')) {
return Promise.resolve(new Response(JSON.stringify({
error: 'Disponível na versão completa.' }), {
status: 403 }));
}
console.error(`[DEMO GUARD] Bloqueada requisição para rota não-mockada: ${urlStr}`);
throw new Error(`Rota ${urlStr}
não foi mockada na Demonstração.`);
}
return originalFetch.apply(this, arguments);
};
...
horario: new Date().toISOString(), umidade: demoData.sensor.umidade, temperatura: demoData.sensor.temperatura, status_bomba: demoData.sensor.bombaStatus
 });
};
...
opacity:0;
pointer-events:none;
transition: opacity 0.25s ease, clip-path 0.3s cubic-bezier(0.16, 1, 0.3, 1), -webkit-clip-path 0.3s cubic-bezier(0.16, 1, 0.3, 1);';
...
popover.innerHTML = `
 <span class="tour-step-info">Passo ${stepIdx + 1}
...
const hasValidData = window._sensorState ? window._sensorState.hasValidData : true;
...
const saude = calcularSaudeSolo(
 window._latestUmid, window._latestSemana, window._latestClima, currentUser.tipoPlantacao, window._sensorState
 );
...
if (et0 !== undefined) {
if (et0 > 5) return {
nivel: et0.toFixed(1) + ' mm', desc: 'Alta perda (FAO). A umidade do solo evapora rapidamente.', cor: 'var(--color-dry)', icon: 'air' };
if (et0 < 2) return {
nivel: et0.toFixed(1) + ' mm', desc: 'Baixa perda (FAO). Condições favoráveis para retenção de água.', cor: 'var(--color-good)', icon: 'cloud' };
return {
nivel: et0.toFixed(1) + ' mm', desc: 'Perda média (FAO). Taxa normal para o período.', cor: 'var(--color-mid)', icon: 'routine' };
}
const temp = clima.current.temperature_2m;
const umid = clima.current.relative_humidity_2m;
if (temp > 28 && umid < 45) return {
nivel: 'Alta', desc: 'Temperatura elevada e baixa umidade do ar aceleram a perda de água do solo.', cor: 'var(--color-dry)', icon: 'air' };
if (temp < 20 || umid > 75) return {
nivel: 'Baixa', desc: 'Condições favoráveis para retenção de água no solo.', cor: 'var(--color-good)', icon: 'cloud' };
return {
nivel: 'Média', desc: 'Taxa de evaporação dentro dos parâmetros normais para o período.', cor: 'var(--color-mid)', icon: 'routine' };
};
const getCropLimits = (tipo) => {
switch ((tipo || '').toLowerCase()) {
case 'milho': return {
min: 40, max: 70, center: 55 };
case 'feijão': return {
min: 40, max: 60, center: 50 };
case 'hortaliças': return {
min: 50, max: 80, center: 65 };
case 'cana-de-açúcar': return {
min: 35, max: 60, center: 47.5 };
default: return {
min: 40, max: 60, center: 50 };
}
};
...
return {
score: '--', status: 'Indisponível', cor: 'var(--sys-light)', mensagem: 'Aguardando dados do sensor.'
 };
}
...
f1 = 100 - (distCenter / halfRange) * 15;
}
else {
...
if (airHumid < 25) f4 -= 20;
else if (airHumid > 90) f4 -= 10;
f4 = Math.max(0, f4);
}
...
if (clampedScore >= 86) return {
score: clampedScore, status: 'Ideal', cor: 'var(--color-good)' };
if (clampedScore >= 71) return {
score: clampedScore, status: 'Bom', cor: '#27ae60' };
if (clampedScore >= 51) return {
score: clampedScore, status: 'Regular', cor: 'var(--color-mid)' };
if (clampedScore >= 31) return {
score: clampedScore, status: 'Baixo', cor: '#f39c12' };
return {
score: clampedScore, status: 'Crítico', cor: 'var(--color-dry)' };
};
...
return `Com a temperatura de ${temp.toFixed(1)}°C, a evaporação da cultura de ${tipoPlantacao.toLowerCase()}
está acelerada. Recomenda-se irrigação.`;
}
...
currentUser = {
id: 'demo123', nome: 'Visitante (Demo)', empresa: 'Fazenda Hidrape', area_hectares: 42, tipoPlantacao: 'Milho', data_cadastro: new Date().toISOString() };
...
try {
btn.disabled = true;
btn.innerHTML = `<span class="material-symbols-rounded" style="animation: rotate 0.8s linear infinite;
font-size: 18px;">progress_activity</span><span>Autenticando...</span>`;
...
const body = {
nome: document.getElementById('reg-nome').value, email: document.getElementById('reg-email').value, senha: document.getElementById('reg-senha').value, cidade: document.getElementById('reg-cidade').value, estado: stateInput ? stateInput.toUpperCase() : 'SP', tipoPlantacao: document.getElementById('reg-cultura').value
 };
try {
btn.disabled = true;
...
b.innerHTML = '<span class="badge-status-dot" style="background:#ff6b6b;
box-shadow: 0 0 8px #ff6b6b"></span> MODO DEMONSTRAÇÃO';
...
type: 'line', data: {
labels: [], datasets: [{
label: 'Umidade (%)', data: [], borderColor: '#94B4C1', backgroundColor: 'rgba(148, 180, 193, 0.05)', fill: true, tension: 0.4, borderWidth: 1.5, pointRadius: 0, pointHoverRadius: 4 }] }, options: {
animation: false, responsive: true, maintainAspectRatio: false, plugins: {
legend: {
display: false }, tooltip: {
backgroundColor: 'rgba(20, 28, 38, 0.9)', titleColor: '#fff', bodyColor: '#94B4C1', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, cornerRadius: 12, padding: 12 }
}, scales: {
x: {
ticks: {
color: 'rgba(255, 255, 255, 0.4)' }, grid: {
display: false }
}, y: {
max: 100, min: 0, beginAtZero: true, ticks: {
color: 'rgba(255, 255, 255, 0.4)' }, grid: {
...
type: 'bar', data: {
labels: [], datasets: [{
label: 'Média Semanal (%)', data: [], backgroundColor: 'rgba(148, 180, 193, 0.4)', borderRadius: 4, hoverBackgroundColor: '#94B4C1' }] }, options: {
responsive: true, maintainAspectRatio: false, interaction: {
mode: 'index', intersect: false }, plugins: {
legend: {
display: false }, tooltip: {
backgroundColor: 'rgba(20, 28, 38, 0.9)', titleColor: '#fff', bodyColor: '#94B4C1', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, cornerRadius: 12, padding: 12 }
}, scales: {
...
color: 'var(--color-good)', fillColor: 'var(--color-good)', fillOpacity: 0.2, radius: 800 
 }).addTo(farmMap);
setTimeout(() => {
if (farmMap) {
...
expanded.push({
data: d.toISOString().split('T')[0], valor: Math.max(0, Math.min(100, single.valor + variance))
 });
}
hist = expanded;
}
...
type: 'line', data: {
labels: hist.map(h => h.data.slice(-5)), datasets: [{
label: 'Umidade Macro (%)', data: hist.map(h => h.valor), borderColor: '#f39c12', backgroundColor: 'rgba(243, 156, 18, 0.2)', fill: true, tension: 0.35, borderWidth: 2, pointRadius: 4, pointHoverRadius: 6, pointBackgroundColor: '#f39c12', pointBorderColor: '#ffffff', pointBorderWidth: 1.5
 }]
 }, options: {
...
legend: {
display: false }, tooltip: {
backgroundColor: 'rgba(20, 28, 38, 0.9)', titleColor: '#fff', bodyColor: '#f39c12', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, cornerRadius: 12, padding: 12, callbacks: {
...
window._sensorState = {
status: 'offline', hasValidData: false };
renderInteligence();
};
let res;
try {
...
color = 'var(--color-dry)';
glowKey = 'dry';
if(iaIcon) iaIcon.textContent = 'water_drop';
}
if (data.status === 'ENCHARCADO') {
color = 'var(--color-dry)';
glowKey = 'dry';
if(iaIcon) iaIcon.textContent = 'flood';
}
if (data.status === 'IDEAL') {
color = 'var(--color-good)';
glowKey = 'good';
if(iaIcon) iaIcon.textContent = 'eco';
}
...
window._sensorState = {
status: 'online', hasValidData: true };
renderInteligence();
}
catch (e) {
console.error('API IoT Error', e);
}
finally {
if (document.getElementById('umidade-valor')) {
liveInterval = setTimeout(updateIoT, 30000);
...
color: white;">✦ IA GENERATIVA</span><br/>
 ${iaData.diagnostico}
`;
}
catch (err) {
...
if (agroData.previsao.tempoHoras === 99) timeText = 'LOCK';
...
color: 'var(--sys-accent, #ff3300)', weight: 2, dashArray: '5, 5', fillOpacity: 0
 }).addTo(farmMap);
farmMap.fitBounds(window._farmBoundary.getBounds(), {
padding: [20, 20] });
const heatPoints = [];
for (let i = 0;
i < 600;
...
console.error('Erro ao buscar RainViewer', e);
}
let hGradient = {
0.4: '#00ff00', 0.65: '#ffff00', 1.0: '#ff0000' };
if (!isCritico && !isAlerta) hGradient = {
0.3: '#00ffff', 0.6: '#00ff00', 1.0: '#00aa00' };
else if (isAlerta) hGradient = {
0.4: '#00ff00', 0.7: '#ffff00', 1.0: '#ff9900' };
...
radius: Math.max(25, 80 - Math.log(tamanhoHectares) * 5), blur: Math.max(20, 50 - Math.log(tamanhoHectares) * 3), maxZoom: 18, minOpacity: 0.4, gradient: hGradient
 });
if (rainUrl) {
window._rainLayer = L.tileLayer(rainUrl, {
maxZoom: 18, maxNativeZoom: 12, opacity: 0.7, zIndex: 10
 });
}
else {
window._rainLayer = L.heatLayer([], {
maxZoom: 18 });
}
...
console.error('Agro Sync Error', e);
}
finally {
if (document.getElementById('clima-cidade')) {
weatherInterval = setTimeout(updateAgro, 10 * 60 * 1000);
}
}
};
updateIoT();
...
document.getElementById('hist-page-info').textContent = `Página ${payload.meta.page}
de ${payload.meta.pages || 1}`;
...
window._targetSettingsTab = null;
}
...
const requestBody = {
nome: document.getElementById('set-nome').value, tipoPlantacao: document.getElementById('set-plantacao').value, cidade: document.getElementById('set-cidade').value, estado: document.getElementById('set-estado').value, tamanhoFazenda: document.getElementById('set-tamanho').value
 };
...
blynkToken: document.getElementById('set-blynk').value, whatsappPhone: document.getElementById('set-phone').value, callmebotApiKey: document.getElementById('set-apikey').value
 })
 });
const updated = await res.json();
...
setTimeout(() => {
document.getElementById('int-success').style.display = 'none';
}, 3000);
}
catch (e) {
alert(e.message);
...
type: 'line', data: {
labels: anHist.map(h => h.data.slice(-5)), datasets: [{
label: 'Umidade Macro (%)', data: anHist.map(h => h.valor), borderColor: '#f39c12', backgroundColor: 'rgba(243, 156, 18, 0.15)', fill: true, tension: 0.35, borderWidth: 2, pointRadius: 4, pointHoverRadius: 6, pointBackgroundColor: '#f39c12', pointBorderColor: '#ffffff', pointBorderWidth: 1.5
 }]
 }, options: {
responsive: true, maintainAspectRatio: false, animation: {
duration: 1000, easing: 'easeOutQuart'
...
legend: {
display: false }, tooltip: {
backgroundColor: 'rgba(20, 28, 38, 0.9)', titleColor: '#fff', bodyColor: '#f39c12', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, cornerRadius: 12, padding: 12 }
}, scales: {
x: {
ticks: {
color: 'rgba(255, 255, 255, 0.5)' }, grid: {
...
if (updateText) updateText.textContent = 'Dados atualizados — ' + new Date().toLocaleTimeString('pt-BR', {
...
type: 'bar', data: {
labels: weekData.map(w => w.dia), datasets: [{
label: 'Média Semanal (%)', data: weekData.map(w => w.media), backgroundColor: 'rgba(148, 180, 193, 0.4)', borderRadius: 4, hoverBackgroundColor: '#94B4C1'
 }]
 }, options: {
responsive: true, maintainAspectRatio: false, interaction: {
mode: 'index', intersect: false }, plugins: {
...
if (evapoNivel) {
evapoNivel.textContent = evapo.nivel;
evapoNivel.style.color = evapo.cor;
}
if (evapoDesc) evapoDesc.textContent = evapo.desc;
if (evapoIcon) evapoIcon.textContent = evapo.icon;
if (evapoIconWrap) {
evapoIconWrap.style.color = evapo.cor;
evapoIconWrap.style.background = evapo.cor + '20';
}
const saude = calcularSaudeSolo(
 window._latestUmid || 50, window._latestSemana || [], window._latestClima, cultura
 );
...
if (saudeStatus) {
saudeStatus.textContent = saude.status;
saudeStatus.style.color = saude.cor;
}
if (saudeProgress) {
saudeProgress.style.strokeDasharray = saude.score + ', 100';
saudeProgress.style.stroke = saude.cor;
}
...
document.getElementById('mr-chuva').textContent = data.kpis.chuvaAcumulada !== null ? `${data.kpis.chuvaAcumulada}
mm` : '-- mm';
document.getElementById('mr-et0').textContent = data.kpis.et0Acumulada !== null ? `${data.kpis.et0Acumulada}
mm` : '-- mm';
...
document.getElementById('mr-volume-teorico').textContent = data.kpis.volumeTeorico !== null ? `${data.kpis.volumeTeorico}
m³` : '-- m³';
...
resumoEl.textContent = `Neste período, o solo permaneceu dentro da faixa adequada em ${data.kpis.pctIdeal}% das leituras observadas. Foram identificados ${data.kpis.recomendacoesCount}
eventos onde a umidade reduziu e alertas foram possivelmente gerados. A ocorrência de precipitação acumulou ${data.kpis.chuvaAcumulada !== null ? data.kpis.chuvaAcumulada : 0}
mm, influenciando o balanço contra uma evapotranspiração teórica de ${data.kpis.et0Acumulada !== null ? data.kpis.et0Acumulada : 0}
mm.`;
}
...
labels: labels, datasets: [
 {
label: 'Umidade do Solo (%)', data: values, borderColor: 'rgba(0, 163, 255, 1)', backgroundColor: 'rgba(0, 163, 255, 0.1)', borderWidth: 2, fill: true, pointRadius: 0
 }
]
 }, options: {
responsive: true, maintainAspectRatio: false, plugins: {
legend: {
display: false }, annotation: {
annotations: {
box1: {
type: 'box', yMin: data.kpis.limites.minIdeal, yMax: data.kpis.limites.maxIdeal, backgroundColor: 'rgba(74, 222, 128, 0.1)', borderWidth: 0
 }
}
}
}, scales: {
y: {
min: 0, max: 100, beginAtZero: true, ticks: {
color: 'rgba(255,255,255,0.5)' }
}, x: {
ticks: {
color: 'rgba(255,255,255,0.5)', maxTicksLimit: 10 }
}
}
}
});
...
labels: labels, datasets: [
 {
label: 'Chuva (mm)', data: chuva, backgroundColor: 'rgba(0, 163, 255, 0.6)', borderRadius: 4
 }, {
label: 'ET₀ (mm)', data: et0, backgroundColor: 'rgba(255, 153, 0, 0.6)', borderRadius: 4
 }
]
 }, options: {
responsive: true, maintainAspectRatio: false, plugins: {
